...
const PaymentButtonComponent = ({ currentPrice, auctionId, setPayIsOpen, amount }) => {
            ...
            try {
                ...
                switch (bootpayResponse.event) {
                    ...
                    case 'confirm': //payload.extra.separately_confirmed = true; 일 경우 승인  전 해당 이벤트가호출됨
                        console.log('bootpayResponse', bootpayResponse);
                        /**
                         * 1. 클라이언트 승인을 하고자 할때
                        ...
                        if (confirmedData.event === 'done') {
                            try {
                                const paymentRes = await paymentCompleteResponse({
                                    receiptId: bootpayResponse.receipt_id,
                                    auctionId: auctionId,
                                    amount: confirmedData.price,
                                });

                                console.log('paymentRes', paymentRes);
                                if (data.success) {
                                    toast(`${useFormattedPrice(response.price)} 원 결제가 완료되었습니다.`);
                                } else {
                                    toast('결제 처리 중 오류가 발생했습니다.');
                                }
                            } catch (error) {
    ...
    return (
        <button type="button" className="btn-set" onClick={() => handlePayment(amount)} disabled={flag}>
            입찰하기
        </button>
    );
};
